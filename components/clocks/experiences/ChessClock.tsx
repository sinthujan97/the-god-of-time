...
export default function ChessClock() {
  ...
  function Panel({ player }: { player: 0 | 1 }) {
    ...
    return (
      ...
      >
        <span style={{ fontFamily: "var(--font-mono)", fontSize: "clamp(38px, 8vw, 72px)", fontWeight: 700, lineHeight: 1, color: timeColor, transition: "color 0.3s" }}>
          {isFlagged ? "FLAG" : fmtSecs(t)}
        </span>
        {isActive && timingMode === "delay" && delayRemaining > 0 && (
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 700, color: "#000000" }}>

...
import { subtractDaysFromDate, AddDaysResult } from "@/lib/tools/calculations";

interface SubtractDaysInputsProps {
  groupAccent: string;
}

export default function SubtractDaysInputs({ groupAccent }: SubtractDaysInputsProps) {
  ...
  useEffect(() => {
    ...
    if (isNaN(amount) || amount === null || amount < 1 || amount > 999999) {
      setResult({
        resultDate: new Date(),
        resultDateFormatted: "—",
        dayOfWeek: "",
        ...
        isLeapYear: false,
        isValid: false,
        errorMessage: "Please enter a positive number starting from 1",
      });
      return;
    }

    const calc = subtractDaysFromDate(formatDateToYYYYMMDD(startDate), amount, unit);
    setResult(calc);
  ...
  const getBreakdownRows = () => {
    if (!result.isValid || !startDate) return [];
    ...
    return [
      ...
      `${amount} ${unit} before ${formattedStart}`,
    ];
  };
  ...
  return (
    ...
    >
      <div className="space-y-6" style={{ "--group-accent": groupAccent } as React.CSSProperties}>
        ...
        {/* Row 2: Amount (Subtract) & Unit */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex flex-col">
            <label className="tool-input-label" htmlFor="subtract-input">
              Subtract
            </label>
            <input
              id="subtract-input"
